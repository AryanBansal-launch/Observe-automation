// Launch Nginx — application-facing noise; stderr/DNS/redis/socket infra lines in launch_nginx_infra.opal
...
| filter log !~ 'unexpected DNS response' and log !~ 'recv() failed' and log !~ 'wrong ident' and log !~ 'send() failed' and log !~ 'Error while connecting to redis: timeout'
